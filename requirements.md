...
- ✅ **Recommendation history: stored (tasks.md 8.0) and now viewable (tasks.md 8.4, done 2026-09-14).** `services/recommendation_history.py` writes one record per served top recommendation and captures thumbs-up/down feedback against it. `get_recent_recommendations` (a single-equality-filter query, 7-day cutoff and sort done in Python — no new composite index needed) backs a new `GET /recommendation/history` route, surfaced in `RecommendationHistoryScreen.tsx` off `ProfileScreen.tsx` — matching README §9.4's own placement under Profile, not Order History.
...
- ✅ **Backend deployed to production (2026-09-14):** `mealsense-api` live on Google Cloud Run — `https://mealsense-api-983327527722.us-west1.run.app`, project `mealsense-cb5ab`, region `us-west1`. Chosen over the README's "Vercel" (written before Firestore existed as the persistence layer) because Cloud Run runs the real long-running ASGI process FastAPI expects, shares the same GCP project as Firestore, and pairs with Cloud Scheduler for the nightly vendor-sync job design-spec.md §7.1 always specified but never had a home for. `mealsense-api/Dockerfile` + `.dockerignore` added (the latter explicitly excludes `.env` — no secret ever enters the build context). Deployed via `gcloud run deploy --source .` (Cloud Build, no local Docker required). See `tasks.md` 7.1 for full detail.
  - Real infrastructure provisioned, not just code: billing enabled on `mealsense-cb5ab`; Cloud Run/Cloud Build/Artifact Registry/Secret Manager APIs enabled; a dedicated least-privilege service account (`mealsense-api-runner@mealsense-cb5ab.iam.gserviceaccount.com`, `roles/datastore.user` + scoped secret access only); `ANTHROPIC_API_KEY` stored in Secret Manager, never as a plain env var.
  - **Two real credential-handling bugs found and fixed as a direct result of deploying** — both invisible to any local testing, since both only manifest with no explicit key file on disk: `services/firestore_client.py` had no Application Default Credentials fallback (would have silently lost all Firestore access on Cloud Run, falling back to the sample menu with no error); `services/auth.py`'s Firebase Admin initialization had the identical gap one layer deeper (would have made every `/admin/*` request fail with a misleading 401 "Invalid or expired token" instead of the real, silently-swallowed `RuntimeError`). Both fixed by falling back to `google.auth.default()` / `firebase_admin.initialize_app()` with no explicit credential — what Cloud Run provides automatically via its metadata server.
  - Live-verified: `/health` and `/menu` (a real query against production Firestore) confirmed working; `/admin/menu/upload` and `/admin/analytics` confirmed correctly returning 401 (not 500) with no auth header. Full end-to-end confirmation of a real kitchen login succeeding against the live service is the natural next check once the mobile app is pointed at the deployed URL on a real device.
- ✅ **Fixed (2026-09-14):** `mealsense-app/src/config/api.ts`'s hardcoded LAN IP — confirmed via earlier live testing to silently go stale on every network switch, with requests just hanging, no error surfaced — replaced with `EXPO_PUBLIC_API_BASE_URL` (matching the existing `EXPO_PUBLIC_FIREBASE_API_KEY` convention), defaulting to the live Cloud Run URL so a production build needs zero configuration.
- ✅ **CI pipeline added (2026-09-14):** `.github/workflows/ci.yml` runs the full test suite on every push/PR to `main` — 246/246 backend tests (real Firestore emulator, not mocked), `tsc --noEmit`, and 25/25 Firestore security-rules tests. See `tasks.md` 7.4 for the emulator-setup detail (deliberately not using `firebase emulators:exec` for the Python job, to avoid breaking the tests that verify the "Firestore not configured" fallback).
- ✅ **Dev formalized, staging deliberately deferred (2026-09-15):** local `uvicorn` + Firestore emulator is the documented dev environment (tasks.md 7.3). A real dev/prod separation bug was found and fixed as part of this — local dev with no emulator running was silently reading real production Firestore via this machine's own `gcloud` Application Default Credentials, left over from the Cloud Run deploy (7.1). Both `services/firestore_client.py` and `services/auth.py`'s bare-ADC fallbacks now require `K_SERVICE` (a Cloud Run-only env var) before trusting ADC at all — off Cloud Run they refuse cleanly instead of silently succeeding against prod. A dedicated staging tier (second Firebase project + Cloud Run service) was scoped but explicitly deferred until there's a real user base to protect — decided with the user, not a silent gap; see tasks.md 7.3 for the full option comparison.
...
| Deployment | ✅ backend live on Cloud Run, app pointed at it, CI running the test suite on push, dev/prod isolation fixed and enforced; staging deliberately deferred to post-launch (§10) |
